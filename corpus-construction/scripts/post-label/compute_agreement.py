#!/usr/bin/env python3
"""
compute_agreement.py — inter-coder agreement for the validation set.

Edit the CONFIG block, then:  python3 compute_agreement.py

Reads every *.jsonl in CODERS_DIR (dp_coder.html exports) plus the author's
own labels from AUTHOR_FILE (`actual_labels`), and writes a metrics report
to OUT_DIR/REPORT.
...
AUTHOR_ID = "author"
INCLUDE_AUTHOR = True        # the author is one of the coders (master_flow: 5 = you + 4)
...
def main() -> int:
    ...
    if author_unknown:
        warn.append(f"{AUTHOR_ID}: codes not in the codebook: "
                    + ", ".join(f"{k} x{v}" for k, v in author_unknown.items()))
